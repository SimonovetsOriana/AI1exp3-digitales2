...
int main(void)
{
	DDRD = (1 << ALARMA);					   // Puerto D: Pulsadores (entrada), alarma y led inidador de enable (salida)
	PORTD = (1 << P1) | (1 << P2) | (1 << P3); // Pull-ups internos. Enable en alto por defecto
	DDRC = 0xFF;							   // Puerto C: Transistores (modo salida)
	DDRA = 0xFF;							   // Puerto B: Displays (modo salida)
	...
	do
	{
		...
		{ // cambió estado del botón
			if (!P1State)
			{ // botón pasó de alto a bajo
				_delay_ms(BOUNCE_DELAY);
				if (!(PIND & (1 << P1)))
				{ // botón sigue en bajo tras retardo
					count += 1;
					if (count > 400)
						count = 50;
				}
	...
	while (1)
	{
		...
		{ // si contador por encima de umbral y alarma no está encendida encender alarma por 10 segundos
			PORTD |= (1 << ALARMA);
			TCCR1B |=
...
ISR(TIMER1_COMPA_vect)
{
	...
	if (timer1_counter >= 5)
	{
		...
		TCCR1B &= ~(1 << CS12) & ~(1 << CS10); // Limpia prescaler y detiene conteo
		timer1_counter = 0;
	}
}
